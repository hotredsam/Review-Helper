...
fn run_once(prompt: String, system: &str) -> Result<String, String> {
...
pub fn generate_questions(term: &str) -> Result<Vec<String>, String> {
    let text = run_once(format!("Concept: {}", term.trim()), PREMADE_SYSTEM)?;
...
pub fn clean_term(input: &str) -> Result<String, String> {
    let text = run_once(format!("Term: {}", input.trim()), CLEAN_SYSTEM)?;
...
pub fn chat_reply(term: &str, what: &str, why: &str, history: &[CardMsg], message: &str) -> Result<String, String> {
    let mut sys = format!(
    ...
    Ok(run_once(message.trim().to_string(), &sys)?.trim().to_string())
}
